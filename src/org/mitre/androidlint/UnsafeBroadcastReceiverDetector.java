...
public class UnsafeBroadcastReceiverDetector extends Detector implements
ClassScanner, XmlScanner {

    /* Description of check implementations:
     *
     * UnsafeBroadcastReceiver check
     *
    ...
    public static final Issue ACTION_STRING = Issue.create(
            ...
            "BroadcastReceiver for protected-broadcast action doesn't check intent action string",
            "BroadcastReceivers that declare an intent-filter for a protected-broadcast action string " +
            "must check before acting that the received intent's action string matches the expected " +
            "value, otherwise it is possible for malicious actors to spoof an intent. (CWE-925)",
            Category.SECURITY,
            6,
            Severity.WARNING,
    ...
    public static final Issue BROADCAST_SMS = Issue.create(
            ...
            "BroadcastReceiver for SMS_DELIVER or SMS_RECEIVED doesn't require BROADCAST_SMS permission",
    ...
    Set<String> receiversToCheck = new HashSet<String>();
    ...
    public void checkClass(@NonNull final ClassContext context,
        ...
        if (!classNode.superName.equals("android/content/BroadcastReceiver")) {
            return;
        }

        // If class isn't in receiversWithIntentFilter (set below by
        // XmlScanner), skip it.
        if (!receiversToCheck.contains(classNode.name)) {
            return;
        }
        ...
        for (Object m : methodList) {
            ...
            if (!method.name.equals("onReceive")) {
                continue;
            }
            ...
            for (int i = 0, n = nodes.size(); i < n; i++) {
                ...
                if (type == AbstractInsnNode.METHOD_INSN) {
                    ...
                    if (node.owner.equals("android/content/Intent")
                            && node.name.equals("getAction")) {
            ...
            if (!getActionEncountered) {
                ...
                if(!intentParameterEncountered) {
                    report = "Ensure that the receiver calls getAction to check that the " +
                             "received intent's action string matches the expected value.";
                } else {
                    // An alternative implementation option is to not report a finding at all in this case, if we
                    // are worried about false positives causing confusion or resulting in developers
                    // ignoring other lint warnings.
                    report = "Ensure that the receiver calls getAction to check that the " +
                             "received intent's action string matches the expected value. " +
                             "In this case, it is possible that the onReceive method passed " +
                             "the received intent to another method that checked the action " +
                             "string. If so, this finding can safely be ignored.";
                }
                context.report(
                    ACTION_STRING,
                    method,
            ...
            for (Element child : children) {
                ...
                if (tagName.equals(TAG_INTENT_FILTER)) {
                    ...
                    for (Element child2 : children2) {
                        if (child2.getTagName().equals("action")) {
                            String actionName = child2.getAttributeNS(
                                    ANDROID_URI, ATTR_NAME);
                            if (actionName != null
                                    && (actionName.equals("android.provider.Telephony.SMS_DELIVER")
                                        || actionName.
                                            equals("android.provider.Telephony.SMS_RECEIVED"))) {
                                if ((permission == null)
                                        || !permission.
                                            equals("android.permission.BROADCAST_SMS")) {
                                    context.report(
                                        BROADCAST_SMS,
                                        element,
                                        context.getLocation(element),
                                        "BroadcastReceivers that declare an intent-filter for " +
                                        "SMS_DELIVER or SMS_RECEIVED must ensure that the caller " +
                                        "has the BROADCAST_SMS permission, otherwise it is " +
                                        "possible for malicious actors to spoof intents.");
                                }
                            }
                            else if (actionName != null
                                    && (Arrays.binarySearch(PROTECTED_BROADCASTS, actionName) 
                                            >= 0)) {
                                receiversToCheck.add(name);
                            }
                        }
                    }
                    break;
